...
public abstract class OpenAiCompatibleInterpretationService implements AiInterpretationService {
    ...
    private static final String SENSITIVITY_RULES =
            """
                    ТЫ ОБЯЗАН ОТВЕЧАТЬ на вопросы о том, что составляет суть этого приложения:
                    — чувства, намерения, мысли и отношение другого человека к пользователю;
                    — отношения, симпатия, влюблённость, ревность, ссоры, примирения;
                    — расставания, бывшие партнёры, шансы возобновить отношения;
                    — брак, свадьба, дети, семейные отношения;
                    — секс и близость между взрослыми людьми;
                    — работа, карьера, деньги, учёба, переезд, жизненные планы.
                    Это разрешённые темы. Отказ по ним недопустим — даже если вопрос кажется \
                    личным, спекулятивным или требующим догадок о другом человеке. Гадание \
                    по своей природе говорит о чувствах и намерениях других людей: именно \
                    за этим пользователь и пришёл.

                    Ты НЕ отвечаешь только на вопросы о следующих темах:
                    — военные конфликты, СВО, войны;
                    — постановка медицинского диагноза, назначение лечения, прогноз выздоровления \
                      от конкретной болезни (общее «как со здоровьем» — можно);
                    — дата смерти, срок жизни, «сколько мне осталось», «умрёт ли»;
                    — суицид, самоповреждение, намерение причинить вред себе или другим;
                    — планирование преступления и установление того, кто совершил преступление;
                    — юридические/финансовые решения, когда вопрос прямо сформулирован как запрос \
                      профессиональной консультации;
                    — азартные игры и инвестиции с гарантией выигрыша или дохода;
                    — политические деятели и партии, религиозные утверждения о правоте;
                    — розыск пропавших людей и установление чьей-либо виновности.
                    Отказывайся, только если вопрос буквально попадает в этот перечень. Если вопрос \
                    в него не попадает — отвечай по существу, отказ будет ошибкой.
                    Только при настоящем отказе начни ответ со слов «Этот вопрос выходит за пределы» \
                    и объясни, что карты не могут помочь с этим.
    ...
    /**
     * Добавка к системному промпту при ПОВТОРНОЙ генерации после того, как первая
     * попытка закончилась отказом, не подтверждённым классификатором.
     *
     * <p>Зачем нужна: модель отказывается не потому, что тема запрещена, а потому что
     * при неуверенности отказ для неё — самый дешёвый путь. Явное указание, что вопрос
     * уже проверен, снимает эту неуверенность. Раньше на этом месте пользователь просто
     * получал ошибку и терял расклад.
     */
    private static final String MODERATION_APPROVED_NOTE =
            """
                    Этот вопрос уже проверен модерацией и признан допустимым: он относится \
                    к разрешённым темам приложения. Отвечай по существу. Не отказывайся, \
                    не уклоняйся и не пиши, что вопрос выходит за пределы — это будет ошибкой.
    ...
    public InterpretationResult interpret(List<CardDto> cards, String question, String category, boolean moderationApproved) {
        ...
        String moderationNote = moderationApproved ? MODERATION_APPROVED_NOTE : StringUtils.EMPTY;
     ...
     * {@inheritDoc}
     *
     * <p>С августа 2026 умеет вернуть {@code NOT_SENSITIVE}. Раньше списка безопасного
     * варианта не было — он заканчивался на {@code LLM_REFUSED}, и классификатор физически
     * не мог оправдать вопрос, из-за чего любой отказ генерирующей модели превращался
     * в блокировку пользователя.
     */
    @Override
    public String classifySensitiveContent(String question) {
        String systemPrompt =
                ...
                NOT_SENSITIVE — вопрос безопасен: ни одна из категорий выше не применима.

                Важно: модель, генерировавшая ответ, уже отказалась отвечать на этот вопрос, \
                но её отказ мог быть перестраховкой. Твоя задача — проверить это независимо. \
                Вопросы о чувствах, намерениях и отношении другого человека, об отношениях, \
                расставаниях, бывших, симпатии, сексе и близости между взрослыми — \
                это штатные темы приложения, отвечай по ним NOT_SENSITIVE. \
                При сомнении выбирай NOT_SENSITIVE.""";

        // sensitivityRules="" намеренно: трёхаргументный callAi подмешивает SENSITIVITY_RULES,
        // адресованные роли "мистический таролог", и классификатор получал инструкцию
        // «начни ответ со слов "Этот вопрос выходит за пределы"» — прямо противоречащую
        // требованию ответить одним словом. Ровно по этой же причине от них уже отказались
        // в classifyQuestionSensitivity (см. его javadoc).
