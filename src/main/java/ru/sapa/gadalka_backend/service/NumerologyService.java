...
public class NumerologyService {
    ...
    /**
     * Резонанс двух чисел 1–9 (или мастер-чисел, которые при расчёте сводятся к диапазону матрицы) —
     * по той же матрице совместимости, что используется для совместимости двух людей.
     * Используется, например, для оценки того, насколько код дня «дружит» с числом жизненного пути человека.
     */
    public int numberAffinity(int a, int b) {
        return compat(a, b);
    }

    /**
     * Редуцирует произвольную сумму цифр до 1–9, сохраняя мастер-числа 11/22/33 —
     * та же логика, что используется для кода дня и числа жизненного пути.
     * Используется, например, для свода кодов всех дней недели в одно «число недели».
     */
    public int reduceToNumerologyNumber(int n) {
        return reduceMaster(n);
    }
